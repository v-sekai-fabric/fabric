...
defmodule Check.Readme do
  ...
  def checks do
    [
      %{
        ...
        kind: :local,
        run: &counts/1,
        break: &Lib.break_readme(&1, "47 projects across 5", "46 projects across 5")
      },
      %{
      ...
      %{
        ...
        break: &Lib.break_readme(&1, "## Checks", "## Checks" <> String.duplicate("\n", 45))
      },
      %{
  ...
  @external %{
    ...
    "weftspun/logbook" => nil,
    "dataflow-coco-gemx/check_readme_claims.py" => "weftspun/dataflow-coco-gemx"
  }
  @runtime MapSet.new([".repo/manifests", "node_modules/.bin", ".repo/"])
  ...
  defp check_token(tok) do
    cond do
      ...
      not File.exists?(Path.join(Lib.root(), tok)) ->
        ["README names `#{tok}`, which does not exist in this repo"]
    ...
    cond do
      ...
      true ->
        String.contains?(tok, "/") and
          File.dir?(Path.join(Lib.root(), tok |> String.split("/") |> hd()))
    end
  end
