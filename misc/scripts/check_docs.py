...
# What a deliverable's own repository says about itself, checked where it can be checked
# now. These replace a gate that asked whether enough seconds had been booked to the mesh in
# thirty days, which was a claim about what should have happened rather than about anything
# that is true. A plan cannot be falsified; it can only be missed, and a build that goes red
# because nobody worked on something is reporting an intention back to the person who had
# it. Everything below is a present-tense fact with a file behind it.
DELIVERABLE_FACTS_OVERRIDE = None
CONVENTION_OVERRIDE = None
LISTING_OVERRIDE = None


def check_no_readme_lists_the_filesystem(mtext, _rtext):
    ...
    the two. It is the same duplication the forty-line rule exists to squeeze out -- a
    second copy of a fact that has an authoritative first copy.

    Detection is grounded rather than guessed. A fenced block counts as a listing when two
    or more of its lines open with a token that names a directory that actually exists in
    that checkout, so an ordinary shell block or a code sample cannot trip it, and a listing
    of directories that were renamed away stops being detected for the same reason it stops
    being true.

    Forks are exempt for the reason they are exempt from the length rule: that README
    belongs to whoever wrote the code.
    """
    if LISTING_OVERRIDE is not None:
        return [f"{name}/README.md lists {n} of its own directories; `ls` already does that"
                for name, n in LISTING_OVERRIDE]
    _, _, projects = parse_manifest(mtext)
    ws = _workspace_root()
    ...
    # A listing is prose formatting and carries no language, which is the difference
    # between the two that the document itself already states.
    fence = re.compile(r"```\n(.*?)```", re.S)
    bad = []
    for p in projects:
        if p["remote"] != OUR_REMOTE or p["name"] in MIRRORS:
            continue
        readme = ws / p["path"] / "README.md"
        if not readme.exists():
            continue
        dirs = {d.name for d in (ws / p["path"]).iterdir() if d.is_dir()}
        hits = 0
        for block in fence.findall(readme.read_text(encoding="utf-8", errors="replace")):
            for line in block.splitlines():
                head = line.strip().split()[0].rstrip("/") if line.strip() else ""
                if head and head.split("/")[0] in dirs:
                    hits += 1
        if hits >= 2:
            bad.append(f"{p['path']}/README.md lists {hits} of its own directories; "
                       "`ls` already does that, and does not go stale")
    return sorted(bad)


def check_the_oracle_has_produced_output(mtext, _rtext):
    """An oracle's recorded output MUST NOT be a byte-for-byte copy of its own reference.

    `data/patches_mesh.json` and `data/hat_mesh.json` are the same blob. A differential
    built on them compares the reference against itself and passes, which is the first
    failure mode this project writes down: a check that cannot fail. The commit that added
    both says they were "generated by triangulate-patches at 100% patch coverage", so they
    are the same mesh from one run rather than a forgery -- and that is exactly why nothing
    noticed. Two names for one file read as two artefacts.

    This is falsifiable today and it goes green the day `run-pipeline` writes a mesh of its
    own, which is the work rather than a wish for it.
    """
    ws = _workspace_root()
    a = ws / "2-contract/patch-verify/data/hat_mesh.json"
    b = ws / "2-contract/patch-verify/data/patches_mesh.json"
    if DELIVERABLE_FACTS_OVERRIDE is not None:
        same = DELIVERABLE_FACTS_OVERRIDE
    else:
        if not (a.exists() and b.exists()):
            return []
        same = a.read_bytes() == b.read_bytes()
    if same:
        return ["patches_mesh.json is byte-identical to hat_mesh.json, so a differential "
                "between them compares the reference against itself and cannot fail"]
    return []


def check_a_convention_matches_its_code(mtext, _rtext):
    """A document that forbids something MUST NOT sit beside code doing it.

    `CHANGELOG.md` states as a convention: "no `@[extern]`/FFI in either direction".
    `Pipeline/Adapters/DmwtAdapter.lean:11` is `@[extern "cassie_triangulate_patch"]`, and
    `CassieGeogram/Delaunay.lean` carries two more. The document is the older of the two and
    was never updated when the FFI arrived, so a reader is told the boundary holds while the
    linker knows otherwise.

    Only this one pairing is checked, because a general rule about prose and code is not
    something a grep can hold. A named claim with a named contradiction is.
    """
    if CONVENTION_OVERRIDE is not None:
        # This check reads a child's files directly rather than through _child_docs, so
        # DOC_OVERRIDE cannot reach it -- a control built on that injected nothing and the
        # check went on failing for the real reason, which reads as a working control and
        # is not one. The reading is replaced instead.
        claim, hits = CONVENTION_OVERRIDE
        return [f"CHANGELOG.md forbids @[extern] and {h} declares one" for h in hits] if claim else []

    ws = _workspace_root()
    doc = ws / "2-contract/patch-verify/CHANGELOG.md"
    if not doc.exists():
        return []
    if "no `@[extern]`/FFI in either direction" not in doc.read_text(encoding="utf-8"):
        return []
    # The attribute is `@[extern "symbol"]`, so a literal `@[extern]` -- which is how the
    # CHANGELOG writes it in prose -- appears nowhere in Lean. Searching for the prose form
    # found nothing and the check passed while three declarations sat in the tree, which is
    # the failure this whole file exists to prevent, arriving inside a check meant to catch
    # it. The open bracket is what the code actually contains.
    hits = sorted(str(f.relative_to(ws)) for f in
                  (ws / "2-contract/patch-verify").rglob("*.lean")
                  if "@[extern" in f.read_text(encoding="utf-8", errors="replace"))
    if hits:
        return [f"CHANGELOG.md forbids @[extern] and {h} declares one" for h in hits]
    return []
...
CHECKS = [
    ...
    ("the oracle has produced output of its own", check_the_oracle_has_produced_output, "local"),
    ("a convention document matches the code it constrains", check_a_convention_matches_its_code, "local"),
...
BREAKAGE = {
    ...
    # Both read files in a child repository, which no edit to the manifest or this README
    # can reach, so the control replaces the reading.
    # Reads children's READMEs against their real directories, which no edit here reaches,
    # so the control replaces the reading.
    "no README lists the filesystem": ("l2", [("transport-somewhere", 7)]),
    "the oracle has produced output of its own": ("o", True),
    # A document that forbids nothing cannot contradict anything, so the control removes
    # the claim rather than the code: with the sentence gone the check must fall silent,
    # which is what proves it reads the document and not just the tree.
    "a convention document matches the code it constrains": (
        "c", (True, ["2-contract/patch-verify/SomewhereElse.lean"])),
...
def main():
    ...
    if self_test:
        ...
        global DEFAULTS_OVERRIDE, FORKS_OVERRIDE, DELIVERABLE_FACTS_OVERRIDE, CONVENTION_OVERRIDE
        ...
        for label, fn, _kind in selected:
            ...
            DELIVERABLE_FACTS_OVERRIDE = CONVENTION_OVERRIDE = LISTING_OVERRIDE = None
            if spec[0] == "l2":
                LISTING_OVERRIDE = spec[1]
            elif spec[0] == "c":
                CONVENTION_OVERRIDE = spec[1]
            elif spec[0] == "o":
                DELIVERABLE_FACTS_OVERRIDE = spec[1]
            elif spec[0] == "f":
