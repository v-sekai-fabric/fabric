...
# A gate states an invariant that holds and must keep holding. It is not the place to record
# a defect: a check that stays red until somebody fixes one particular thing is an issue
# tracker in a test's clothes, and a build that is permanently red teaches everyone to stop
# reading red. Two such checks lived here -- an oracle output identical to its own reference,
# and a CHANGELOG forbidding an @[extern] that three files declare -- and both are now
# fire/contract-patch-verify#3 and #4, which is where a defect belongs.
